...
import {
  clearDraftMockDecklist,
  loadDraftMockDecklist,
  loadDraftMockSetupState,
  updateDraftMockSetupSummary,
  type DraftMockSetupState,
} from '@/arkham/draft/storage'
...
const draftMockSetupState = ref<DraftMockSetupState | null>(loadDraftMockSetupState(props.gameId))
const draftMockSetupSummaryDismissed = ref(false)
...
const autoResolveDraftMockStandaloneSettings = computed(
  () => draftMockSetupState.value?.autoResolveStandaloneSettings === true,
)
const draftMockSetupSummary = computed(
  () => draftMockSetupState.value?.standaloneSetupSummary ?? [],
)
const showDraftMockSetupSummary = computed(
  () => draftMockSetupSummary.value.length > 0 && !draftMockSetupSummaryDismissed.value,
)
...
const onError = () => {
  ...
  if (loadDraftMockDecklist(props.gameId)) {
...
const { send, close, status: websocketStatus } = useWebSocket(websocketUrl, {
  autoReconnect: true,
  onError,
  onConnected,
  onMessage,
...
async function maybeSubmitDraftMockDecklist(): Promise<void> {
  ...
  if (websocketStatus.value !== 'OPEN') return
...
watch([game, playerId, websocketStatus], () => {
  const decklist = loadDraftMockDecklist(props.gameId)
  if (
    decklist &&
    game.value &&
    Object.values(game.value.investigators).some(
      (investigator) => investigator.cardCode === decklist.investigator_code,
    )
  ) {
...
watch(
  ...
  () => {
    submittedDraftMockDecklist.value = false
    draftMockSetupState.value = loadDraftMockSetupState(props.gameId)
    draftMockSetupSummaryDismissed.value = false
  },
)
...
async function onDraftMockSetupRandomized(summary: string[]) {
  updateDraftMockSetupSummary(props.gameId, summary)
  draftMockSetupState.value = loadDraftMockSetupState(props.gameId)
  draftMockSetupSummaryDismissed.value = false
    ...
    <template v-else>
      ...
      <div v-else class="game-main">
        ...
        <ScenarioSettings
          ...
          :playerId="playerId"
          :autoResolve="autoResolveDraftMockStandaloneSettings"
          @randomized="onDraftMockSetupRandomized"
        />
        <StandaloneScenario
          ...
          @toggleRealityAcidLight="toggleRealityAcidLight"
        />
        <aside v-if="showDraftMockSetupSummary" class="draft-setup-summary">
          <div>
            <strong>Draft setup randomized</strong>
            <ul>
              <li v-for="entry in draftMockSetupSummary" :key="entry">{{ entry }}</li>
            </ul>
          </div>
          <button type="button" @click="draftMockSetupSummaryDismissed = true">Dismiss</button>
        </aside>
        <div
          class="sidebar"
